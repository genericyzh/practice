 ...
 * 给出一个不重复的数组，将其打乱
 * <p>
 * 参考：https://www.kancloud.cn/kancloud/data-structure-and-algorithm-notes/72928
 ...
 * 而对于card[i]之前的元素，以索引j处的元素card[j]为例进行分析可知，其在位置j的概率为1/(i-1) * (i-1)/i = 1/i,
 * 具体含义为遍历到索引i-1时card[j]位于索引j的概率(1/(i-1))乘以遍历到索引i时随机数未选择与索引j的数进行交换的概率((i-1)/i).
 * 需要注意的是前面的j <= i-1,
 * 那么card[j]位于索引i的概率又是多少呢？要位于索引i，则随机数k须为i, 这种概率为1/i.
 * <p>
 * PS:在第i轮交换中，j在数组中的位置都为 1/i，
 * i = 1: 1/1
 * i = 2: 1/2
 * i = 3: 1/3
 * ...
